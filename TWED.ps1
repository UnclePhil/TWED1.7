...
##fixed definition
$pkoversion = "TiddlyEsxDoc 1.7 Unclephil-201103"
$crlf="`n"
...
$filelocation=$reportdir+"\Cmdb."+$vcserver+"."+$Filetime+".html"
...
Write-Host "==========="
Write-Host "Vsphere server : $vcserver"
Write-Host "User : $uc"
...
$vs =Connect-VIServer $vcserver -port $portvc -User $uc -Password $pw
If ($vs.IsConnected -ne $true){
	Write-host "Cannot connect to Virtual center : " $vcserver
	exit
}
Write-Host "Start Analyzing"
...
######################################################################################################
## Generation of CMDBTitle Tiddler
###############################
Write-Host "Generating Summarizing tiddler ====="
$fields = ""
$ct = $vcserver +" "+$Starttime+$crlf
...
$fields = ""
$ct = ""
$ct += "!Generation of this cmdb"+$crlf
...
$ct += "!Generator"+$crlf
$ct += "*"+$pkoversion+$crlf
$toinject = TiddlerBuild "GenerationInfos" "Info" $fields $ct
##inject tiddler in file
Tiddlyadd $toinject $filelocation
